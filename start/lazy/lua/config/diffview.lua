...
local diffview_max_count = 8

local filehistory = function()
  if vim.opt.modifiable:get() == false or vim.opt.diff:get() == true then
    return
  end
...
local quit = function()
  ...
  local tabpagecnt = vim.fn.tabpagenr('$')
  vim.cmd('DiffviewClose')
  if tabpagenr > 1 and tabpagecnt > vim.fn.tabpagenr('$') then
    vim.cmd(string.format([[norm %dgt]], tabpagenr - 1))
  end
end
...
diffview.setup(get_table())

local toggle_history_cnt = function()
  if diffview_max_count == 32768 then
    diffview_max_count = 8
  else
    diffview_max_count = 32768
  end
  diffview.setup(get_table())
  print('g:diffview_max_count:', diffview_max_count)
end

vim.keymap.set({ 'n', 'v' }, '<leader>gi', filehistory, { silent = true, desc = 'diffview filehistory' })
vim.keymap.set({ 'n', 'v' }, '<leader>go', open, { silent = true, desc = 'diffview open' })
vim.keymap.set({ 'n', 'v' }, '<leader>gq', quit, { silent = true, desc = 'diffview quit' })

vim.keymap.set({ 'n', 'v' }, '<leader>gT', toggle_history_cnt, { silent = true, desc = 'diffview toggle_history_cnt' })
